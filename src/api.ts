...
import { getFileById, deleteFileRecord, listFilesWithSearch, listFilesAdvanced, getDashboardStats, getSetting, setSetting } from './db';
...
adminApi.get('/file/list', async (c) => {
  ...
  const { files, total } = await listFilesAdvanced(env.DB, page, size, keyword, dateFrom, dateTo, sort);
  return ok(c, {
    page,
    size,
    total,
